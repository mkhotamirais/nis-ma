<x-authlayout title="Update Berita">
    ...
    <form action="{{ route('news.update', $news) }}" method="POST" enctype="multipart/form-data" class="">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-input !w-full @error('title') !border-red-300 @enderror" name="title"
                id="title" value="{{ $news->title }}" placeholder="Judul berita">
            @error('title')
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content" cols="30" rows="10"
                class="input @error('content') !ring-red-500 @enderror">{{ $news->content }}</textarea>
            @error('content')
                <p class="error">{{ $message }}</p>
            @enderror
        </div>
        ...
        @if ($news->banner)
            ...
            <figure class="h-40 w-64 rounded-md mb-4 overflow-hidden">
                <img id="current-image" src="{{ asset('storage/' . $news->banner) }}"
                    alt="{{ $news->title ?? 'News Image' }}" width="400" height="400"
